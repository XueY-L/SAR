...
CUDA_VISIBLE_DEVICES=0 python3 main_imagenetc_5kSamples_forget.py --exp_type normal --model resnet50_bn_torch
"""
import copy
...
if __name__ == '__main__':
    ...
    dataset = ImageFolder(f'/home/yxue/datasets/ImageNet-C/{source}/1', transforms.Compose([transforms.Resize(256), transforms.CenterCrop(224), transforms.ToTensor()]))
    ...
    for corrupt in common_corruptions:
        ...
        if args.method in ['tent', 'eata', 'sar', 'no_adapt']:
            if args.model == "resnet50_gn_timm":
                ...
                net.load_state_dict(torch.load(f'/home/yxue/model_fusion_tta/imagenet/checkpoint/ckpt_[\'{source}\']_[1].pt')['model'])
                args.lr = (0.00025 / 64) * bs * 2 if bs < 32 else 0.00025
            else:
                assert False, NotImplementedError
            net = net.cuda()
        ...
        temp_optim = copy.deepcopy(adapt_model.optimizer.base_optimizer)
        temp_model = copy.deepcopy(adapt_model)
        temp_model.optimizer.base_optimizer = temp_optim
        for data, label in loader:
            ...
            num_cor += correct
        acc1s.append((num_cor/len(loader)).item())
